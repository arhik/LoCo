...
class FullVisionModel(torch.nn.Module):
    ...
    def forward(self, x, label, n=4):
        ...
        loss = torch.zeros(1, self.opt.model_splits-1, device=cur_device) #first dimension for multi-GPU training
        accuracies = torch.zeros(1, self.opt.model_splits-1, device=cur_device) #first dimension for multi-GPU training

        for idx, module in enumerate(self.encoder[: n-1]): # n+1 should be n
            moduleA = self.encoder[idx]
            moduleB = self.encoder[idx+1]
            zA,  n_patches_x, n_patches_y = moduleA(
                model_input, n_patches_x, n_patches_y, label
            )
            z, n_patches_x, n_patches_y = moduleB(
                zA, n_patches_x, n_patches_y, label
            )
